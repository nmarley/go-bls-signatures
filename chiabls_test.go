...
//		t.Run(fmt.Sprintf("%d", i), func(st *testing.T) {
//			is := is.New(st)
//			pk, _ := chiabls.DeserializePublicKey(tt.publicKey)
//			sig, _ := chiabls.DeserializeSignature(tt.signature)
//
//			mh := chiabls.NewMessageHashFromBytes(chiabls.Hash256(tt.payload))
//			sig.SetAggregationInfo(chiabls.AggregationInfoFromMsgHash(pk, mh))
//			is.True(sig.Verify())
//		})
//	}
//}
...
func TestVectorHDKeys(t *testing.T) {
	...
	chainCode := chiabls.ChainCodeFromBytes([]byte{
	...
	esk := chiabls.ExtendedSecretKeyFromSeed(seed)
	is.Equal(esk.GetPublicKey().Fingerprint(), pkFingerprint)
	is.Equal(esk.GetChainCode(), chainCode)
	...
	is.Equal(esk77.GetChainCode(), chainCode)
