...
func main() {
	...
	sk1 := bls.SecretKeyFromBytes(sk1Bytes)
	sk2 := bls.SecretKeyFromBytes(sk2Bytes)

	sig1 := sk1.Sign(m1)
	sig2 := sk2.Sign(m2)

	insecureSig1, _ := bls.SignatureFromBytes(sig1Bytes)
	insecureSig2, _ := bls.SignatureFromBytes(sig2Bytes)

	aggSig := bls.AggregateSignatures([]*bls.Signature{sig1, sig2})
	quot1 := aggSig.DivideBy([]*bls.Signature{sig1})
	fmt.Printf("quot1: %x\n", quot1.Serialize())
	fmt.Printf("sig2: %x\n", sig2.Serialize())

	aggSigIns := bls.AggregateSignaturesSimple([]*bls.Signature{insecureSig1, insecureSig2})
	quotIns1 := aggSigIns.DivideBy([]*bls.Signature{insecureSig1})
	fmt.Printf("quotIns1: %x\n", quotIns1.Serialize())
	fmt.Printf("insecureSig2: %x\n", insecureSig2.Serialize())
}
